...
uniform positionalLightStruct light;
...
void main(void) {
	...
	vec4 litColour = vec4(tmp_Color.rgb *attenuation, tmp_Color.a);						//Attenuation does not affect transparency
	vec4 amb=min(ambientI,vec4(1.0f));
	
	//out_Color=min((litColour + totalLight) + amb*texture(textureUnit0, ex_TexCoord),vec4(1.0f));			//Here attenuation does not affectambient
	
	
	out_Color = ( litColour + totalLight) * texture(textureUnit0, ex_TexCoord),vec4(1.0f);
